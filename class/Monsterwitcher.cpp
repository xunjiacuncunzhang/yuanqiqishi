...
void Monsterwitcher::InitMonsterwitcherSprite(char* name, char* xue_back, char* xue_fore)
{
	...
	Monster_xue->setPosition(ccp(m_MonsterwitcherSprite->getPositionX() + 25, m_MonsterwitcherSprite->getPositionY() + 50));//设置在怪物上头  
...
void Monsterwitcher::FollowRun(CCNode* m_hero, CCNode* m_map)
{
	...
	if (x < -10)//判断怪物横坐标和英雄的距离
	{
		...
		this->SetAnimation("monster_run", 6, MonsterwitcherDirecton);//播放动画
