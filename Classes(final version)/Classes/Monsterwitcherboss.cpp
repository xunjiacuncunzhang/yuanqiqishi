...
Monsterwitcherboss* Monsterwitcherboss::createWithFileName(const char* name)//重载create
{
...
void  Monsterwitcherboss::StopAnimation()
{
	if (!IsRunning)
		return;
	m_MonsterwitcherbossSprite->stopAllActions();//当前精灵停止所有动画
	//恢复精灵原来的初始化贴图 
	this->removeChild(m_MonsterwitcherbossSprite, TRUE);//把原来的精灵删除掉
	m_MonsterwitcherbossSprite = CCSprite::create(Monster_name);//恢复精灵原来的贴图样子
...
void Monsterwitcherboss::FollowRun(Sprite* m_hero, TMXTiledMap* m_map)
{
	...
	if (x < -100)//判断怪物横坐标和英雄的距离
	{
		...
		m_MonsterwitcherbossSprite->setFlipX(MonsterwitcherDirecton);//设置方向
		if (IsAttack)
			return;
...
void Monsterwitcherboss::StartListen(Sprite* m_hero, TMXTiledMap* m_map)
{
	my_hero = m_hero;
	my_map = m_map;
	this->schedule(schedule_selector(Monsterwitcherboss::updateMonster), 3.0f);//每隔3秒计算距离
